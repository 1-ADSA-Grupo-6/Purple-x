function carregarGraficos() {
    ...
    function plotarGrafico(resposta) {
        ...
        for (let i = 0; i < resposta.length; i++) {
            let dadosMaquina = resposta[i];
            labels.push(dadosMaquina.nome); 
            dados.datasets[0].data_102.push(dadosMaquina.registro);
        }
        ...
        graficoSmith = new Chart(grafico_102, config);
        setTimeout(() => atualizarGrafico(dados, myChart), 2000);
    }

    function obter() {
        fetch("/dadosGraficos/obter", { cache: 'no-store' }).then(function (response) {
            if (response.ok) {
    ...
    function atualizarGrafico(dados, myChart) {
        fetch("/dadosGraficos/obter", { cache: 'no-store' }).then(function (response) {
            if (response.ok) {
                response.json().then(function (novodadosMaquina) {
                    console.log(`Dados recebidos: ${JSON.stringify(novodadosMaquina)}`);
                    console.log(`Dados atuais do gráfico:`);
                    console.log(dados);

                    if (novodadosMaquina.length > 0 && novodadosMaquina[0].totalPontuacao !== dados.labels[dados.labels.length - 1]) {
                        if (!dados.labels.includes(novodadosMaquina[0].nome)) {
                            if (dados.labels.length >= 5) {
                                dados.labels.pop();
                                dados.datasets[0].data.pop();
                            }
                            dados.labels.unshift(novodadosMaquina[0].nome);
                            dados.datasets[0].data.unshift(novodadosMaquina[0].totalPontuacao);
                            myChart.update();
                        } else {
