function esconderMenu() {
    menu_lateral.innerHTML = `
        <div id="esconder_menu" class="div_menu_oculto" onclick="mostrarMenu()">
        <img src="./img/iconMenu-quadrado.png" class="lateral_icon">
        </div>`
    menu_lateral.classList.add('menu_oculto')
    carrocel.style.width = '95.2%'
    carrocel.style.marginLeft = '4.8%'
}
function mostrarMenu() {
    menu_lateral.classList.remove('menu_oculto')
    carrocel.style.width = '100%'
    carrocel.style.marginLeft = '0%'
    menu_lateral.innerHTML = `
        <div id="container_menu_lateral">   
            <div id="esconder_menu" class="menu" onclick="esconderMenu()">
                <img src="./img/iconMenu-quadrado.png" class="lateral_icon">
            </div>
            <!-- </label> -->
            <div id="trocar_academia" class="menu">
                <img src="./img/gym-icon-white.svg" class="lateral_icon">
            </div>
            <div id="adicionar_maq" class="menu">
                <img src="./img/add-icon-white.svg" class="lateral_icon">
            </div>
            <div id="remover_maq" class="menu">
                <img src="./img/sub-icon-white.svg" class="lateral_icon">
            </div>
            <div id="remover_maq" class="menu">
                <img src="./img/sino-icon-white.svg" class="lateral_icon">
            </div>
        </div>
        <a href="../Login e Cadastro/login.html" class="menu">
        <div id="logout">
                <img src="./img/sair-icon-white.svg" class="lateral_icon">
            </div>
        </a>`
        
}